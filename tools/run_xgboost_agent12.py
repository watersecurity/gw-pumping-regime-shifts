#!/usr/bin/env python3
"""
Individual XGBoost Model for Agent 12 — POST-CP Regime (test_start=2014).

Trains individual models on Agent 12 only (no pooling) with dedicated
...
Baseline to beat: Pooled model test R²=-0.071, RMSE=49.6 mm.
"""
...
OUTPUT_DIR = RESULTS_DIR / "two_regime_c1" / "agent12_individual"
...
def load_and_prepare():
    """Load Agent 12 data, aggregate to annual, build POST-CP dataset."""
...
def plot_best_model(tv, te, best_entry, output_dir):
    ...
    for fmt, dpi in [("pdf", 300), ("png", 150)]:
        fig.savefig(output_dir / f"agent12_point.{fmt}",
                    dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    print(f"\nPlot saved to {output_dir}/agent12_point.pdf/.png")
...
def main():
    ...
    print(f"  Pooled baseline: R²=-0.071  RMSE=49.6 mm")
